...
<?endif?>
<?$arQues = array(47,48,49, 50, 51);?>
<div class="alert alert-info">
  <strong>Внимание!</strong> Поля, отмеченные звездочкой, обязательны для заполнения.
</div>
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" id="registration_kigr">
	<?=bitrix_sessid_post()?>
	...
	<div class="kigr">
		...
		<tbody>
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
				<?if ($propertyID == "PREVIEW_PICTURE") continue;?>
				<div class="row">

				<?if(in_array($propertyID, $arQues)):?>
					<div class="col-md-6">
						<?if ($propertyID == 47):?><h3>Анкета участника:</h3><?endif;?>
				<?else:?>
					<?if ($propertyID == 40):?>
						<div class="col-md-12">
							<h3>Паспортные данные участника (необходимы для заселения в гостиницу):</h3>
						</div>
						<div class="col-md-2">
					<?else:?>
						<div class="col-md-2">
				...
				<?if ($propertyID==35) $propertyID = "PREVIEW_PICTURE";?>

					<?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?><span class="starrequired">*</span><?endif?></div>
					...
					<div class="col-md-10">
						...
						switch ($INPUT_TYPE):
							...
							case "T":
								...
								?>
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea><br><br>
								<?
								}
							break;
							...
							case "N":
								...
								<?if($propertyID=="46"):?>	
									<input type="email" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>" value="<?=$value?>" />
								<?else:?>
		...
		<?endif?>
			<div class="row"><br>
				<div class="col-md-12">
					<input type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" class="btn btn-primary btn-lg">
