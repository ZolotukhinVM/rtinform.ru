...
// if ( $APPLICATION->CaptchaCheckCode($cp_word, $csid) ) {
    if (is_array($file['add_file']) && $file['add_file']['error'] == 0) {
        ...
        $rmail = new PHPMailer();
        $rmail->CharSet = "utf-8";
        $rmail->setFrom('noreply@rtinform.ru', 'rtinform.ru');
        // $rmail->addAddress('hr@rtinform.ru');
        $rmail->addAddress('v.zolotukhin@rtinform.ru');
        //$rmail->AddBCC('v.zolotukhin@rtinform.ru');
